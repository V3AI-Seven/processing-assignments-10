...
func stop_game() -> void: #linked with signal
	for timer in timers:
		timer.stop()
...
#connected to various timers to trigger attack animations
...
func sliding_box_3() -> void:#linked via signal
	$SlidingBox3/AnimationPlayer.play("SlidingBox1/SlidingBox") #don't mind the weird name, it works, but I don't really know why it got named like that
